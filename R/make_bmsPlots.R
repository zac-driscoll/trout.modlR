...
#'
#' @return Error checks
run_bms_error_check <- function(dat1,dat2,dat3,type){
  #check the type argument
  if(!type %in% c("single","multiple","cumulative")){
    stop(paste("Please provide a valid plot type.",
               "Options include 'single','multiple',",
               "or 'cumulative'"))
  }
  #check that two+ datasets are provided when type != single
  if((is.null(dat2) & is.null(dat3)) &
      type != "single"){
    stop(paste("You must provide more than one model",
               "when type is equal to 'multiple' or 'cumulative'"))
  }
  #check that one datasets is provided when type != single
  if((!is.null(dat2) | !is.null(dat3)) &
     type == "single"){
    stop(paste("Only provide a one model",
               "when type is equal to 'single'"))
  }
}
